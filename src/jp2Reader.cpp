...
jp2Reader::jp2Reader(DD::Image::Read *iop)
...
void jp2Reader::engine(int y, int x, int r, DD::Image::ChannelMask mask, DD::Image::Row &row)
{
  if (mFIF != FIF_UNKNOWN)
  {
    if (!mBitmap)
...
static bool test(int fd, const unsigned char* block, int length)
{
  ...
  return (memcmp(jp2_signature, block, sizeof(jp2_signature)) == 0);
}

static DD::Image::Reader* build(DD::Image::Read* iop, int fd, const unsigned char* b, int n)
{
  return new jp2Reader(iop);
}

const DD::Image::Reader::Description jp2Reader::Desc("jp2\0", build, test);
